...
int GzGetDisplay(GzDisplay *display, int i, int j, GzIntensity *r, GzIntensity *g, GzIntensity *b, GzIntensity *a, GzDepth *z)
{
	...
	GzPixel pixel = display->fbuf[j*display->xres + i];
	*r = pixel.red;
	*g = pixel.green;
	*b = pixel.blue;
	*a = pixel.alpha;
...
int GzFlushDisplay2File(FILE* outfile, GzDisplay *display)
{
	...
	for (int i = 0; i < y; i++){
		for (int j = 0; j < x; j++){
			fprintf(outfile, "%d %d %d ", display->fbuf[i*x + j].red, display->fbuf[i*x + j].green, display->fbuf[i*x + j].blue);
		}
	}
	return GZ_SUCCESS;
}
